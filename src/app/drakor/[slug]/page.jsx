...
async function getDramaDetail(slug) {
  ...
  // Slug from route params might be "detail/to-my-beloved-thief-2026-gdbb/" or "/detail/to-my-beloved-thief-2026-gdbb/"
  // Ensure it has leading slash and /detail/ prefix
  let cleanSlug = slug;
  if (!cleanSlug.startsWith('/')) {
    cleanSlug = `/${cleanSlug}`;
  }
  // If slug doesn't start with /detail/, it means it's just the slug part, so add /detail/ prefix
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {episodes && episodes.length > 0 ? (
                  episodes.map((episode, index) => (
                    <Link
                      href={`/drakor/watch/${episode.slug || index}?slug=${slug}&title=${encodeURIComponent(drakor.title || '')}&image=${encodeURIComponent(drakor.image || '')}`}
                      key={`episode-${episode.slug || index}-${index}`}
