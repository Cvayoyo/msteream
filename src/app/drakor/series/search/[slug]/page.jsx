...
async function searchSeriesDrakor(slug) {
    if (!slug) return [];

    try {
        const baseUrl = getBaseUrl();
        const searchUrl = `${baseUrl}/api/series/search?q=${slug}`;
        ...
        console.error("Gagal total saat mengambil hasil pencarian series:", error);
        return [];
    }
}
...
export default async function SearchPage({ params: ParamsPromise }) {
    ...
    const searchResults = await searchSeriesDrakor(slug);

    return (
        <div className="min-h-screen bg-[#1A1A29] text-white">
            <div className="container mx-auto px-4 py-8">
                <div className="mb-10">
                    <Header title={`Hasil Pencarian Series: ${keyword}`} />
                    <div className="mt-6 flex flex-col items-center">
                        <SearchInput basePath="/drakor/series/search" placeholder="Cari Drakor Series lain..." />
                    </div>
                </div>
                ...
                {searchResults && searchResults.length > 0 ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-20 bg-neutral-800/20 rounded-3xl border border-neutral-700/50 backdrop-blur-sm">
                        ...
                        <h2 className="text-2xl md:text-3xl font-semibold text-neutral-400 text-center mb-2">
                            Tidak ditemukan di Drakor Series...
                        </h2>
